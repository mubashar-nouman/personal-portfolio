        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {services.map((service) => {
            ...
            return (
              <article
                key={service.id}
                className="group relative overflow-hidden rounded-xl border border-zinc-200 bg-white p-4 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-orange-300 hover:shadow-md dark:border-zinc-700 dark:bg-zinc-800 dark:hover:border-orange-500/50"
              >
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <h3 className="text-sm font-bold leading-snug text-zinc-900 dark:text-white">{service.title}</h3>
                </div>

                <p className="mt-3 text-xs leading-relaxed text-zinc-600 dark:text-zinc-300">{service.description}</p>

                <ul className="mt-3.5 space-y-1.5 border-t border-zinc-100 pt-3.5 dark:border-zinc-700">
                  {service.deliverables.map((deliverable) => (
                    <li
                      key={deliverable}
                      className="flex items-start gap-2 text-xs text-zinc-600 dark:text-zinc-300"
                    >
                      <Check size={13} className="mt-0.5 flex-shrink-0 text-orange-500" />
                      {deliverable}
                    </li>
                  ))}
                </ul>
